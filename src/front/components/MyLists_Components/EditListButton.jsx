...
export const EditListButton = ({ list }) => {
  ...
  const closeModal = () => {
    const modalEl = document.getElementById(`editListBtnModal-${list.id}`); // tu id exacto
    if (modalEl) {
  ...
  const updateListStatus = (listId, store) => {
    ...
    const allCompleted = tasksOfList.every(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (store.token) {
        const data = await listServices.updateList(list.id, formData);
        ...
        if (data.success) {
          ...
          showError(
            data.error || "Could not updated the list, please try again."
          );
        }
      } else {
  ...
  return (
    <div className="col-12 col-md-3 d-flex justify-content-end me-3">
      <button
        ...
        data-bs-target={`#editListBtnModal-${list.id}`}
      ...
      <div
        ...
        id={`editListBtnModal-${list.id}`}
        tabIndex="-1"
        aria-labelledby="editListBtnModalLabel"
        aria-hidden="true"
      >
